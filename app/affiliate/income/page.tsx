...
export default function AffiliateIncomePage() {
  ...
  const getDefaultSales = (pkg: number) => {
    ...
    const sales: number[] = []
    for (let i = 0; i < 3; i++) {
      const distribution = INCOME_DISTRIBUTION[i] ?? 0
      const commissionRate = COMMISSION_RATES[i] ?? 0
      const levelIncome = targetIncome * distribution
      const commissionPerSale = pkg * commissionRate
  ...
  const calculateIncome = () => {
    return salesByLevel.reduce((total, sales, index) => {
      const commissionRate = COMMISSION_RATES[index] ?? 0
      return total + sales * selectedPackage * commissionRate
    }, 0)
  }
  ...
  const calculateSalesForGoal = () => {
    ...
    for (let i = 0; i < 3; i++) {
      const distribution = INCOME_DISTRIBUTION[i] ?? 0
      const commissionRate = COMMISSION_RATES[i] ?? 0
      const levelIncome = goalIncome * distribution
      const commissionPerSale = selectedPackage * commissionRate
