...
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (dealerError) {
      console.error("[Admin Suspend Dealer Error]", dealerError)
      return NextResponse.json({ error: "Failed to suspend dealer" }, { status: 500 })
    }
    ...
    console.error("[Admin Suspend Dealer Error]", error)
    return NextResponse.json({ error: "Failed to suspend dealer" }, { status: 500 })
  }
}
