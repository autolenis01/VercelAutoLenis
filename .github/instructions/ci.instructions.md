...
## Do
- Use `pnpm install --frozen-lockfile` (via `corepack pnpm` if pnpm is unavailable) and commit lockfile-only dependency changes.
- Run `pnpm lint`, `pnpm test`, and `pnpm build` (runs `prisma generate` before `next build`) with required envs from `.env.example`; stub or guard `POSTGRES_PRISMA_URL` so builds don’t hard-fail when DB access is unavailable.
